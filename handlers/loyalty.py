...
from utils.line_registry import bind as registry_bind, get_phone, get_name, TEMP_PREFIX
...
async def loyalty_start(user_id: str):
    ...
    # "line:<user_id>" — временная привязка (ensure_bound), ещё не настоящий телефон
    if phone and phone.startswith(TEMP_PREFIX):
        phone = None
    if phone:
